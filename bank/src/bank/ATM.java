...
//		은행 선택
		...
		String menu = "1. 계좌개설\n2. 입금하기\n3. 출금하기\n4. 잔액조회\n5. 계좌번호 찾기\n6. 은행선택메뉴로 이동";
		//account는 아래에서 연결하여 사용하기 때문에 null이 아닌 빈 문자열값을 저장
		...
		while(true) {
			...
			while(true) {
				...
				switch(choice) {
				case 1: //계좌개설
					//관리하기 편하게 배열 사용. 규칙성 없는 객체들 사이에 규칙성 부여
					//배열이 Bank 타입이기 때문에 업캐스팅. 
					Bank[] arBank = {new Shinhan(), new Kookmin(), new Dongseok()};

					while(true) {
						//초기화 안하면 계좌개설마다 계속 누적됨. 그래서 새 계좌 만들 때마다 초기화 시켜줌
						account = "";
						//13자리 계좌번호
						//계좌 맨 앞에 은행번호를 넣을 거라 12번 반복
						for (int i = 0; i < 12; i++) {
							//""에 0~9 사이의 정수값이 임의로 연결
							account += r.nextInt(10);
						}
	//					쓸데없이 객체화를 해야 할 때에는 빨리 static으로 전환한다!
	//					Bank bank = new Bank();
	//					bank.checkAccount()
						//Bank 클래스에 있는 checkAccount나 다른 메소드들 모두 static
					...
					//은행 객체를 넣어둔 arBank 배열. bankNumber-1로 해당 인덱스에 있는 객체를 가져옴.
					//거기에 .을 써서 접근하고 setter를 사용해 account에 접근
					...
					//은행 객체를 넣어둔 arBank 배열. bankNumber-1로 해당 인덱스에 있는 객체를 가져옴.
					//거기에 .을 써서 접근하고 setter를 사용해 name에 접근
